...
export class WorldMapScene extends Phaser.Scene {
  ...
  private _buildGameTiles(): void {
    ...
    for (const { tile, isCity, display, x, y, pts, terrColor, terrainVariant } of localTiles) {
      ...
      zone.on('pointerdown', () => {
        ...
        if (this._interactionMode === 'air' && this.gsm.airEnemies.some(e => e.hexId === tile.id)) {
          this._openAirCombatLaunch(tile);
        } else if (isCity) { this._openCityView(); }
  ...
  private _renderHintLine(W: number, H: number, hintH: number): void {
    ...
    // Air / Ground mode toggle button — right side of hint bar.
    this.modeToggleBtn = this.add.text(W - 14, H - hintH / 2, '[ GROUND ]', {
      fontSize: '13px', color: '#44ffaa', fontFamily: 'monospace', fontStyle: 'bold',
      backgroundColor: '#003322cc', padding: { x: 8, y: 4 },
  ...
  private _makeEnemyMarkerVisual(container: Phaser.GameObjects.Container): void {
    const gfx = this.add.graphics();
    gfx.fillStyle(0xff2222, 0.28); gfx.fillCircle(0, 0, 9);
    gfx.fillStyle(0xff3333, 1.0);  gfx.fillCircle(0, 0, 5);
    gfx.fillStyle(0xffffff, 0.55); gfx.fillCircle(-1.5, -1.5, 1.8);
    container.add(gfx);
  ...
  private _toggleInteractionMode(): void {
    ...
    this.modeToggleBtn
      ...
      .setBackgroundColor(isAir ? '#331100cc' : '#003322cc');
